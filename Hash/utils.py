...
from option.options import HiDDenConfiguration, TrainingOptions
...
def get_data_loaders(hidden_config: HiDDenConfiguration, train_options: TrainingOptions):
    ...
    validation_images = datasets.ImageFolder(train_options.validation_folder, data_transforms['val'])
...
def get_testdata_loaders(path,batch_size):
    ...
    test_images = datasets.ImageFolder(path, data_transforms['val'])
    test_loader = torch.utils.data.DataLoader(test_images, batch_size=batch_size, shuffle=False, num_workers=0)
